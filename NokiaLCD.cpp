...
void NokiaLCD::LCDsetArc(int x0, int y0, int radius, int arcSegments[], int numSegments, int lineThickness, int color)
{
	...
	for(int i = 0; i < lineThickness; i++)
	{
		...
		while(x < y)
		{
			...
			{
				if(arcSegments[i] == NNE) LCDSetPixel(x0 - y, y0 + x,color); //SHOW NNE
				if(arcSegments[i] == ENE) LCDSetPixel(x0 - x, y0 + y,color); //SHOW ENE
				if(arcSegments[i] == ESE) LCDSetPixel(x0 + x, y0 + y,color); //SHOW ESE
				if(arcSegments[i] == SSE) LCDSetPixel(x0 + y, y0 + x,color); //SHOW SSE
				if(arcSegments[i] == SSW) LCDSetPixel(x0 + y, y0 - x,color); //SHOW SSW
				if(arcSegments[i] == WSW) LCDSetPixel(x0 + x, y0 - y,color); //SHOW WSW 
				if(arcSegments[i] == WNW) LCDSetPixel(x0 - x, y0 - y,color); //SHOW WNW
				if(arcSegments[i] == NNW) LCDSetPixel(x0 - y, y0 - x,color); //SHOW NNW
			}
